...
int main()
{
    ...
    long i, j, m, t1, t2, indx, nfrm, nuni, poly, mono, *step, *dfrm, *dcnt, *dmap, lsze, fsze;
    FILE *cord, *fram, *msdm, *msdc;

    //  INITIALIZE ALGORITHM VARIABLES
    char base_[] = "DATA_ETHY_T046_10/";              //  INPUT: DATA DIRECTORY
    char fram_[] = "fram_ETHY_T046_10_CNF0";          //  INPUT: SIMULATION TIME STEPS
    char cord_[] = "bmon_ETHY_T046_10_CNF0";          //  INPUT: MONOMER COORDINATES IN BINARY
    char dirn_[] = "MDIX_ETHY_T046_10/";              //  OUTPUT: RESULTS DIRECTORY
    char msdm_[] = "msdm_ETHY_T046_10_CNF0";          //  OUTPUT: MONOMER DISPLACEMENT/ALPHA
    char msdc_[] = "msdc_ETHY_T046_10_CNF0";          //  OUTPUT: COM DISPLACEMENT/ALPHA
    poly = 120;                                       //  NUMBER OF POLYMERS
    mono = 10;                                        //  NUMBER OF MONOMERS PER POLYMER
    fact = 0.002;                                     //  TIME CONVERSION FACTOR
    ...
    //  CALCULATE AND STORE TIME INTERVALS
    dfrm = lvectr(0,step[nfrm]);
    for(i=0; i<=step[nfrm]; i++)
    {
        dfrm[i] = 0;
    }
    for(i=1; i<=nfrm; i++)
        ...
        for(i=1; i<=mono; i++)
        {
            ...
            for(t1=1; t1<=nfrm; t1++)
            {
                for(t2=t1; t2<=nfrm; t2++)
                {
                    ...
                    indx = dfrm[step[t2]-step[t1]];
                    dsqr[indx] = dsqr[indx] + prod;
                    dqrt[indx] = dqrt[indx] + prod*prod;
                }
            }
        }
        so = time(NULL);
    ...
    for(t1=2; t1<=nuni; t1++)
    {
        dsqr[t1] = dsqr[t1]/(double)(poly*mono*dcnt[t1]);
        dqrt[t1] = dqrt[t1]/(double)(poly*mono*dcnt[t1]);
        alfa = 3.0/5.0*dqrt[t1]/(dsqr[t1]*dsqr[t1]) - 1.0;
        fprintf(msdm, "% 20.16E\t% 20.16E\t% 20.16E\t% 20.16E\n",
            fact*(double)dmap[t1], dsqr[t1], dqrt[t1], alfa);
    }
    fclose(msdm);
    ...
    for(t1=2; t1<=nuni; t1++)
    {
        dsqr[t1] = dsqr[t1]/(double)(poly*dcnt[t1]);
        dqrt[t1] = dqrt[t1]/(double)(poly*dcnt[t1]);
        alfa = 3.0/5.0*dqrt[t1]/(dsqr[t1]*dsqr[t1]) - 1.0;
