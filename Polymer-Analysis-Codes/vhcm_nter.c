//  ROUTINE COMPUTES INTERMOLECULAR CENTER OF MASS VAN HOVE FUNCTION
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <time.h>
...
int main()
{
    ...
    for(m=1; m<=cnts; m++)
    {
        sprintf(pgcr_, "%s%s_D%li", dirn_, gcrt_, delt[m]);
        gcrt = fopen(pgcr_, "w");
        tnum = 0;
        for(t1=1; t1<=nfrm; t1++)
        {
        ...
        for(i=0; i<nbin; i++)
        {
            ...
            hist[m][i] = hist[m][i]
                       / (double)(poly*poly*tnum*volu*invb*invb*invb);
            fprintf(gcrt, "% 20.16E\t% 20.16E\n", (double)((i+0.5)*delr), hist[m][i]);
        }
        fclose(gcrt);
    }
