//  ROUTINE COMPUTES INTERMOLECULAR MONOMER/CENTER-OF-MASS?
...
int main()
{
    ...
    double **xcor, **ycor, **zcor, *xcom, *ycom, *zcom, *hist, boxs, delr, volu,
        xdis, ydis, zdis, dist, invd, invb, rmax, shex, shin, scal;
    long i, m, n, t, nbin, mono, poly, nfrm, lsze, ibin;
    FILE *cord, *fram, *hofr;

    //  INITIALIZE ALGORITHM PARAMETERS
    char base_[] = "DATA_ETHY_T453_96/";              //  INPUT: DATA DIRECTORY
    char fram_[] = "fram_ETHY_T453_96";               //  INPUT: SIMULATION TIME STEPS
    char cord_[] = "bmon_ETHY_T453_96";               //  INPUT: BINARY SITE COORDINATES
    char dirn_[] = "HOFR_ETHY_T453_96/";              //  OUTPUT: RESULTS DIRECTORY
    char hofr_[] = "hcmr_ETHY_T453_96";               //  OUTPUT: TOTAL COORELATION FUNCTION
    poly = 1600;                                      //  NUMBER OF CHAINS
    mono = 96;                                        //  NUMBER OF MONOMERS PER CHAIN
    delr = 0.2;                                       //  HISTOGRAM BIN WIDTH
    boxs = 167.270;                                   //  BOX LINEAR DIMENSION
    ...
    for(t=1; t<=nfrm; t++)
    {
        ...
        for(m=1; m<poly; m++)
        {
            for(n=m+1; n<=poly; n++)
            {
                for(i=1; i<=mono; i++)
                {
                    xdis = xcor[m][i] - xcom[n];
                    ydis = ycor[m][i] - ycom[n];
                    zdis = zcor[m][i] - zcom[n];
    ...
    for(i=0; i<nbin; i++)
    {
        ...
        hist[i] = hist[i]
                    / (double)(mono*poly*poly*nfrm*volu*invb*invb*invb);
