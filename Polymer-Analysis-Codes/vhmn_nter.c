//  ROUTINE COMPUTES INTERMOLECULAR MONOMER VAN HOVE FUNCTION
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <time.h>
...
int main()
{
    ...
    long *delt, i, j, k, m, n, t1, t2, ibin, mono, poly, nbin,
    ...
    char gmrt_[] = "gmer_ETHY_T450_0100";             //  OUTPUT: VAN HOVE FUNCTION
                        ...
                        for(n=1; n<=poly; n++)
                        {
                            if(n!=m)
                            {
                                for(i=1; i<=mono; i++)
                                {
                                    for(j=1; j<=mono; j++)
                                    {
                                        xdis = xcr2[n][j] - xcr1[m][i];
                                        ydis = ycr2[n][j] - ycr1[m][i];
                                        zdis = zcr2[n][j] - zcr1[m][i];
