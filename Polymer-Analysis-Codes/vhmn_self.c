//  ROUTINE COMPUTES SELF MONOMER VAN HOVE FUNCTION
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <time.h>
...
int main()
{
    ...
    long *delt, i, k, m, t1, t2, ibin, mono, poly, nbin,
    ...
    char gmrt_[] = "gmsf_ETHY_T450_0100";             //  OUTPUT: VAN HOVE FUNCTION
            ...
            for(k=1; k<=cnts; k++)
            {
                if((stmp[t2]-stmp[t1])==delt[k])
                {
                    ...
                    for(m=1; m<=poly; m++)
                    {
                        for(i=1; i<=mono; i++)
                        {
                            xdis = xcr2[m][i] - xcr1[m][i];
                            ydis = ycr2[m][i] - ycr1[m][i];
                            zdis = zcr2[m][i] - zcr1[m][i];
